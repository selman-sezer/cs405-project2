...
class MeshDrawer {
	...
	setMesh(vertPos, texCoords, normalCoords) {
		 ...
		 */
		// change
		gl.bindBuffer(gl.ARRAY_BUFFER, this.normalbuffer); // Binding normal buffer
    	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(normalCoords), gl.STATIC_DRAW); // Adding normals data
	...
	draw(trans) {
		...
		///////////////////////////////
		gl.bindBuffer(gl.ARRAY_BUFFER, this.normalbuffer); // Binding normal buffer
    	gl.enableVertexAttribArray(this.normalLoc); // Enabling normal attribute
    	gl.vertexAttribPointer(this.normalLoc, 3, gl.FLOAT, false, 0, 0);

    	gl.uniform3f(this.lightSourceLoc, this.lightX, this.lightY, 1.0);

    	// Enabling lighting based on checkbox value
    	gl.uniform1i(this.enableLightingLoc, document.getElementById('enable-light').checked);

	    // Passing ambient light value from slider
	    gl.uniform1f(this.ambientLoc, document.getElementById('ambient-light-setter').value / 100);
...
const meshFS = `
			...
			void main()
			{
				if(showTex && enableLighting){
            		...
            		vec3 lightDir = normalize(lightPos - v_normal); // Calculate
            		//float diff = max(dot(v_normal, lightDir), 0.0);
                	float diff = max(dot(normalize(v_normal), lightDir), 0.0);
                	vec3 diffuse = diff * texColor.rgb;

                	//vec3 diffuse = vec3(diff) * texColor.rgb;
